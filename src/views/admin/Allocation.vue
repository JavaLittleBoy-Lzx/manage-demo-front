  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-location"></i> 调拨申请
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-form :inline="true" :model="query" class="demo-form-inline" label-width="60px">
          ...
          <el-button type="primary" @click="handleAdd">调拨申请
          </el-button>
          ...
          <el-table-column label="操作" width="180" align="center" fixed="right">
            <template #default="scope">
              <el-button type="text" icon="el-icon-edit" @click="handleEdit(scope.row)"
                :disabled="(scope.row.auditStatus === 1) && (applicantUserId == scope.row.applicantUserId) ? false : true">编辑
              </el-button>
              <el-button type="text" icon="el-icon-delete" class="red"
                @click="handleDelete(scope.$index, scope.row.allocationId)"
                :disabled="(scope.row.auditStatus === 1 && applicantUserId == scope.row.applicantUserId) ? false : true">删除
              </el-button>
            </template>
          </el-table-column>
        </el-table-column>
      ...
      <el-dialog title="申请" v-model="dialogVisible" width="50%">
        <el-form :model="form.data" ref="formRef" :rules="rules" label-width="100px">
          <el-form-item label="设备名">
            <el-select v-model="form.data.deviceId" placeholder="请选择设备" @change="getDepartment">
              <el-option v-for="item in deviceList" :key="item.deviceId" :label="item.deviceName" :value="item.deviceId"
                :disabled="isdepartmentId != item.departmentId ? false : true">
          ...
          <el-form-item label="审批人">
            <el-select v-model="form.data.auditUserId" placeholder="请选择审批人">
    ...
    <div>
      ...
      <div>
        <el-dialog title="申请编辑" v-model="editVis" width="50%">
          <el-form :model="form.data" ref="formRef" :rules="rules" label-width="100px">
            <el-form-item label="设备名">
...
const save = () => {
  ...
  formRef.value.validate((valid) => {
    if (valid) {
      var method = form.data.allocationId === "" ? "POST" : "PUT";
      request({
        url: "/parking/allocation",
        method: method,
        data: form.data,
      }).then((res) => {
        form.data = {}
