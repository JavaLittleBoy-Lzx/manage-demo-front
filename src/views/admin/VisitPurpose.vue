<template>
  <div class="visit-purpose-management">
    <!-- 现代化页面头部 -->
    <div class="page-header">
      <div class="page-header-content">
        <div class="page-title">
          <div class="title-icon">
            <img src="..//../icons/svg-black/VisitPurpose.svg" alt="来访目的" class="header-icon" />
          </div>
          <div class="title-text">
            <h1>来访目的管理</h1>
            <p class="title-desc">管理来访人员的目的分类，支持自定义排序和快速检索</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="container">
          ...
          <div v-show="searchPanelExpanded" class="search-panel-body">
            <el-form :inline="true" :model="query" class="search-form" label-width="80px">
              ...
              <div class="search-row">
                <el-form-item label="来访目的" class="search-item">
                  <el-input 
                    v-model="query.reason" 
                    placeholder="请输入来访目的关键字" 
                    clearable
                    class="search-input"
                  />
                </el-form-item>
      ...
      <div class="table-panel">
        ...
        <div class="table-panel-body">
          ...
          >
            <!-- 来访目的列 -->
            <el-table-column label="来访目的" prop="reason" min-width="200">
              <template #default="{ row }">
                <div class="purpose-info-cell">
                  <div class="purpose-icon">
                    <el-icon><User /></el-icon>
                  </div>
                  <div class="purpose-details">
                    <div class="purpose-name">{{ row.reason }}</div>
                    <div class="purpose-desc">访客来访目的</div>
                  </div>
                </div>
              </template>
            </el-table-column>
...
const handleSearch = async () => {
  try {
    ...
    if (query.reason) searchConditions.push(`来访目的: ${query.reason}`);
    
    const searchDescription = searchConditions.length > 0 
        ? `查询来访目的 (${searchConditions.join(', ')})` 
        : '查询所有来访目的';
    
    await activityApi.logVisitPurposeOperation(
        userId,
        username,
        "查询",
        searchDescription,
...
const handleDelete = (index, sid) => {
  // 获取要删除的来访目的信息，用于记录日志
  const visitPurposeInfo = tableData.value[index];
  
  // 二次确认删除
  ElMessageBox.confirm("确定要删除这个来访目的吗？", "删除确认", {
    type: "warning",
      ...
      try {
        ...
        if (res.data) {
          ...
          await activityApi.logVisitPurposeOperation(
            ...
            username,
            "删除",
            `删除了来访目的：${visitPurposeInfo.reason}`,
            sid,
            JSON.stringify(visitPurposeInfo),
            null
          ).catch(err => {
...
.dark {
  ...
  .purpose-info-cell .purpose-icon {
    background: linear-gradient(135deg, #1e40af 0%, #7c3aed 100%);
  }
}
