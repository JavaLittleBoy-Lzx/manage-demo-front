  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-location"></i> 调拨审批
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
        ...
        <el-table-column label="审批状态" prop="auditStatus" align="center">
          <template #default="scope">
            <el-tag type="success" v-if="scope.row.auditStatus === 1">待审批</el-tag>
            <el-tag type="warning" v-else-if="scope.row.auditStatus === 2">已通过</el-tag>
            <el-tag type="success" v-else-if="scope.row.auditStatus === 3">未通过</el-tag>
          </template>
        </el-table-column>
        <el-table-column>
          <el-table-column label="操作" width="180" align="center" fixed="right">
            <template #default="scope">
              <el-button type="text" icon="el-icon-edit" @click="handleEdit(scope.row)"
                :disabled="scope.row.auditStatus === 1 ? false : true">审核
              </el-button>
            </template>
          </el-table-column>
        </el-table-column>
    ...
    <div>
      <el-dialog title="审核情况" v-model="dialogVisible" width="50%">
        <el-form :model="form.data" ref="formRef" :rules="rules" label-width="100px">
          <!--            <el-form-item label="设备名">-->
          <!--              <el-input v-model="form.data.deviceName" style="width: 80%" disabled></el-input>-->
          <!--            </el-form-item>-->
          <!--            <el-form-item label="设备编码">-->
          <!--              <el-input v-model="form.data.deviceCode" style="width: 80%" disabled></el-input>-->
          <!--            </el-form-item>-->
          ...
          <el-form-item label="审批意见">
            <el-input type="textarea" :rows="2" v-model="form.data.fileReason" style="width: 80%"></el-input>
          </el-form-item>
        </el-form>
        <template #footer>
...
const props = [
  ...
  { label: "原部门", prop: "departmentName" },
  { label: "调往部门", prop: "afterDepartmentName" },
  ...
  { label: "审批时间", prop: "auditusTime" },
  { label: "备注", prop: "remarks" },
];
const form = reactive({
  data: {
    allocationId: '',//zhujian
    deviceId: "",//设备id
    afterDepartmentId: localStorage.getItem("departmentId"),//调往部门
...
const getData = () => {
  query.auditUserId = localStorage.getItem("userId")
  request
    .get(root + "pageByAuditUserId", {
      params: query,
    })
    .then((res) => {
...
const getDevice = () => {
  request.get("/parking/device/listByTypeOne").then((res) => {
...
const save = () => {
  form.data.applicantUserId = applicantUserId
  form.data.departmentId = localStorage.getItem("departmentId")
  request.post("/parking/allocation/updateAllocation", form.data).then((res) => {
    if (res.code === null) {
      getData()
