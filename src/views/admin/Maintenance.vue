  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-cascades"></i> 报修申请
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-form :inline="true" :model="query" class="demo-form-inline" label-width="60px">
          ...
          <el-button type="primary" @click="handleAdd">报修申请
          </el-button>
          ...
          <el-table-column label="操作" width="180" align="center" fixed="right">
            <template #default="scope">
              <el-button type="text" icon="el-icon-edit" @click="handleEdit(scope.row)"
                :disabled="(scope.row.auditStatus === 1) && (applicantUserId == scope.row.repairmanUserId) ? false : true">编辑
              </el-button>
              <el-button type="text" icon="el-icon-delete" class="red"
                @click="handleDelete(scope.$index, scope.row.maintenanceId)"
                :disabled="(scope.row.auditStatus === 1) && (applicantUserId == scope.row.repairmanUserId) ? false : true">删除
              </el-button>
      ...
      <el-dialog title="申请" v-model="dialogVisible" width="50%">
        <el-form :model="form.data" ref="formRef" :rules="rules" label-width="100px">
          <el-form-item label="设备名">
            <el-select v-model="form.data.deviceId" placeholder="请选择设备">
              <el-option v-for="item in deviceList" :key="item.supplierId" :label="item.deviceName"
                :value="item.deviceId">
                <span style="float: left">{{ item.deviceName }}</span>
                <span style="  float: right;   color: #777777;   font-size: 15px;  ">{{ item.deviceCode }}</span>
                <!--              </el-option>-->
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="故障障描述">
            <el-input type="textarea" :rows="2" v-model="form.data.faultDescription" style="width: 80%"></el-input>
          </el-form-item>

          <el-form-item label="维修人">
            <el-select v-model="form.data.maintenanceUserId" placeholder="请选择审批人">
...
const handleEdit = (row) => {
  ...
  form.data.maintenanceId = row.maintenanceId
  form.data.deviceId = row.deviceId
  // alert(row.deviceId)
  form.data.maintenanceUserId = row.maintenanceUserId
  form.data.faultDescription = row.faultDescription
...
const save = () => {
  form.data.repairmanUserId = applicantUserId
  // 表单校验
  formRef.value.validate((valid) => {
    if (valid) {
      var method = form.data.maintenanceId === "" ? "POST" : "PUT";
      request({
        url: "/parking/maintenance",
        method: method,
        data: form.data,
      }).then((res) => {
        form.data = {}
